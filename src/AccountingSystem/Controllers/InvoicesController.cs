...
public class InvoicesController : CrudControllerBase
{
    ...
    private async Task<bool> ValidateAndComputeAsync(InvoiceFormViewModel model)
    {
        ...
        for (var i = 0; i < model.Details.Count; i++)
        {
            ...
            if (detail.IsSerialControlled)
            {
                ...
                if (detail.SelectedSerialIds.Count == 0)
                {
                ...
                if (!detail.CustomerWarrantyEndDate.HasValue)
                {
